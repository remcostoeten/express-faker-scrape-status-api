...
class Application {
  static generateApplications() {
    const discounters = [
      'bol', 'kaufland', 'lidl', 'praxis', 'hp', 'samsung', 'coolblue', 'kruidvat',
      'plein', 'ochama', 'trekpleister', 'amazon', 'pepper', 'tones', 'darty',
      'fnac', 'muller'
    ];
    ...
    const applications = discounters.flatMap(discounter => {
      if (['bol', 'coolblue', 'kruidvat', 'plein', 'ochama', 'trekpleister', 'tones', 'darty', 'fnac', 'muller'].includes(discounter)) {
        return {
  ...
  static getByState(state) {
    ...
    return apps.filter(app => app.state === state)
